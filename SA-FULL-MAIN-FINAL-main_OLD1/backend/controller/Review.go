...
// // GetReviewsByProductID - ดึงรีวิวของสินค้าตาม productID
// func GetReviewsByProductID(c *gin.Context) {
//     productID := c.Param("product_id") // ดึง productID จากพารามิเตอร์ URL

//     var reviews []entity.Review
//     db := config.DB()

//     // ดึงรีวิวที่เชื่อมโยงกับสินค้าตาม productID
//     result := db.Where("products_id = ?", productID).Find(&reviews)
...
//     if len(reviews) == 0 {
//         c.JSON(http.StatusNotFound, gin.H{"message": "ไม่มีรีวิวสำหรับสินค้านี้"})
//         return
//     }
