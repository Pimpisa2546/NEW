...
//review
  ...
  // async function GetReviewByProductID(productID: number) {
  //   try {
  //     const response = await axios.get(`${apiUrl}/review/${productID}`);
  //     return response.data;
  //   } catch (error) {
  //     // ใช้ type assertion เพื่อลดข้อผิดพลาดนี้
  //     if (axios.isAxiosError(error)) {
  //       // จัดการข้อผิดพลาดจาก axios
  //       return { error: error.response?.data.message || "Error fetching reviews" };
  //     } else {
  //       // กรณีเป็นข้อผิดพลาดอื่น ๆ ที่ไม่ใช่ axios
